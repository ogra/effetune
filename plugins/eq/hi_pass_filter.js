...
// --- Calculate Linkwitz-Riley high-pass filter coefficients ---
...
// Coefficients for 2nd-order Linkwitz-Riley high-pass filter
const b0 = n;
const b1 = -2 * n;
const b2 = n;
...
for (let ch = 0, offset = 0; ch < channelCount; ch++, offset += blockSize) {
  for (let i = 0; i < blockSize; i++) {
    ...
    // Apply cascaded Linkwitz-Riley high-pass filters
    for (let s = 0; s < numStages; s++) {
...
class HiPassFilterPlugin extends PluginBase {
  ...
  createUI() {
    ...
    freqRow.appendChild(createSlopeSelect(this.slope, v => this.setSlope(v), "HPF"));
