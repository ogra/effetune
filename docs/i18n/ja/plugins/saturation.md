...
- [Intermodulator](#intermodulator) - ハーモニック・インターモジュレーション効果により、倍音の相互作用で音に新たなキャラクターを追加
...
ハーモニック・インターモジュレーションは、入力信号に対して意図的に異なる倍音成分を加えることで、音に独特の歪みと豊かな音色をもたらすエフェクトです。従来のサチュレーションとは異なり、倍音同士の相互作用により、従来では得られない新たな音の表情を創出します。
...
### パラメータ
- **2nd Harm (%):** 2次倍音量の調整（0〜30%、デフォルト: 2%）  
  - 低設定で控えめな効果、高設定で明確な倍音強調
- **3rd Harm (%):** 3次倍音量の調整（0〜30%、デフォルト: 3%）  
  - 音に輝きと明瞭さを追加
- **4th Harm (%):** 4次倍音量の調整（0〜30%、デフォルト: 0.5%）  
  - 繊細な倍音補強により、音のニュアンスを拡張
- **5th Harm (%):** 5次倍音量の調整（0〜30%、デフォルト: 0.3%）  
  - 軽い設定で複雑さをプラスし、微妙な色味を演出
- **Sensitivity (x):** 感度の調整（0.1〜2.0、デフォルト: 0.5）  
  - 低感度では穏やかな倍音生成、高感度では顕著な歪み効果を発揮
