...
- **HPF Freq (Hz)** - 共鳴から不要な低周波数を除去するハイパスフィルター（20から20,000 Hz）。
  - 低い値：より多くの低周波数成分を通過させます。
  - 高い値：低音周波数を除去することで共鳴を薄くします。
  - LPFと組み合わせて使用することで、より精密な周波数帯域制御が可能です。
  - デフォルト値は各レゾネーターの周波数の半分に設定されています。

- **Gain (dB)** - 各レゾネーターの個別出力レベルを制御します（-18から+18 dB）。
  - 負の値：レゾネーターの出力レベルを下げます。
  - -12 dB：デフォルトのゲイン設定。
  - 正の値：レゾネーターの出力レベルを上げます。
  - 異なるレゾネーター間のバランスを微調整するのに役立ちます。
